...
FREQS = list(fftfreq(8192, 1.0/DEFAULT_FS))
...
class WaveDataComparator:
    ...
    def makeFingerprints(self, file):
        ...
        for n in range(len(segList)):
            segList[n] = (self.fingerprint(segList[n][0]), segList[n][1], file[1])
    ...
    def fingerprint(self, samples, bands = BANDS):
        ...
        # to their magnitudes
        samples = map(abs, list(fft(samples)))
...
#####################################################################
    ...
    def areMatching(self):
        ...
        for x in wave1list:
            ...
            x = self.fingerprintB(x, waveDataFormat1.sample_rate, DEFAULT_WINDOW_SIZE,
                                 DEFAULT_OVERLAP_RATIO, DEFAULT_FAN_VALUE,
                                 DEFAULT_AMP_MIN)
            #print x
            for y in wave2list:
                #y = self.makeFftList(y, float(waveDataFormat2.sample_rate))
                y = self.fingerprintB(y, waveDataFormat2.sample_rate, DEFAULT_WINDOW_SIZE,
                                     DEFAULT_OVERLAP_RATIO, DEFAULT_FAN_VALUE,
                                     DEFAULT_AMP_MIN)
    ...
    def fingerprintB(self, channel_samples, Fs=DEFAULT_FS,
